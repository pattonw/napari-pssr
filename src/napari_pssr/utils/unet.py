...
def image_from_tiles(learn, img, tile_sz=128, scale=4):
    pimg = PIL.Image.fromarray((img * 255).astype(np.uint8), mode="L").convert(
        "RGB"
    )
    cur_size = pimg.size
    ...
    in_img = Image(
        pil2tensor(
            pimg.resize(new_size, resample=PIL.Image.BICUBIC), np.float32
        ).div_(255)
    )
    c, w, h = in_img.shape
    ...
    for x_tile in range(math.ceil(w / tile_sz)):
        for y_tile in range(math.ceil(h / tile_sz)):
            ...
            ] = in_img.data[:, x_start:x_end, y_start:y_end]
            ...
            out_img[
                :, out_x_start:out_x_end, out_y_start:out_y_end
            ] = out_tile.data[:, in_x_start:in_x_end, in_y_start:in_y_end]
    return out_img
